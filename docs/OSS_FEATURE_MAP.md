...
This table tracks every open-source project that informed GraphCtx v0.2.0. Columns:
...
|---|---|---|---|---|---|---|---|
| AutoMem | https://github.com/verygoodplugins/automem | `_refs/automem` | MIT | 11 relationship taxonomy, 9-signal scoring, bridge discovery, LoCoMo/LongMemEval benchmark discipline | implemented | Relation enum and benchmark evaluation approach inspired by AutoMem's typed-relationship model and scoring discipline | none |
...
| Nocturne Memory | https://github.com/nicholasgriffintn/nocturne-memory | `_refs/nocturne_memory` | MIT | Rollbackable MCP memory, local persistent agent context, SQLite/Postgres storage | schema-hook | Rollback/delete test patterns for MCP memory operations; local MCP ergonomics for persistent agent context | none |
...
| sqlite-vec | https://github.com/asg017/sqlite-vec | N/A | MIT | Local vector search as a SQLite extension | v0.3 | Potential optional vector acceleration backend; not required in v0.2 | none |
| Kuzu | https://github.com/kuzudb/kuzu | N/A | MIT | Embedded graph database with Cypher-like query language | v0.3 | Potential optional graph backend; not required in v0.2 | none |
...
- **implemented** -- concept already present in GraphCtx v0.2.0
...
- **v0.3** -- deferred to v0.3 (optional backends, advanced features)
